...
function phaseLabel(phase: string): string {
  const stripped = phase.replace(/^phase_\d+_/, "");
...
export default async function DashboardPage() {
  ...
  return (
    <div className="space-y-8">
      <PageHeader
        title="Faith & Sports Ministry"
        subtitle="An internal planning tool for church leaders. Start with one small meeting, then let it grow."
        action={<LinkButton href="/initiatives/new">New initiative</LinkButton>}
      />
        ...
        <CardBody>
          {initiatives.length === 0 ? (
            <EmptyState
              ...
              action={
                <LinkButton href="/initiatives/new">Create your first initiative</LinkButton>
              }
            />
          ) : (
      ...
      <Card>
        ...
        <CardBody>
          {upcomingEvents.length === 0 ? (
            <EmptyState
              title="No events yet"
              hint="Plan a meeting or workshop from within an initiative."
            />
          ) : (
        ...
        <CardBody>
          {pendingReviews.length === 0 ? (
            <EmptyState
              ...
              hint="All reviews are up to date."
            />
          ) : (
            <ul className="divide-y divide-slate-100">
              {pendingReviews.map((review) => {
                ...
                const href =
                  review.kind === "theology" ? "/reviews/theology" : "/reviews/pastoral";
                return (
                  <li key={review.id}>
                    <Link
                      href={href}
                    ...
                    >
                      <div className="flex min-w-0 items-center gap-2">
                        <Badge
                          tone={review.kind === "theology" ? "blue" : "amber"}
                        >
